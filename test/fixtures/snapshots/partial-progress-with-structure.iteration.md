...
## Template Selection
Snapshot fixture for populated structure definition context.
...
- src/ (source): Production TypeScript source files.
- test/ (test): Node test suites.
- docs/ (docs): Durable product and architecture docs.
- File placement rules:
  - src/**/*.ts → src (Implementation code stays under src/.)
  - test/**/*.test.ts → test (Regression coverage belongs in test/.)
- Forbidden paths (do not create or modify):
...
- Prior follow-up action: continue_same_task
