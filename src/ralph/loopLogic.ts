...
export function decideLoopContinuation(input: RalphStopDecisionInput): RalphLoopDecision {
  ...
  const verifierSuspectCount = countTrailingMatches(
    history,
    (item) =>
      item.selectedTaskId === input.currentResult.selectedTaskId
      && (item.agentId ?? DEFAULT_RALPH_AGENT_ID) === agentId
  ...
  if (
    input.currentResult.executionStatus === 'succeeded'
    ...
    const repeatedValidationFailureCount = countTrailingMatches(
      history,
      (item) =>
        item.selectedTaskId === input.currentResult.selectedTaskId
        && (item.agentId ?? DEFAULT_RALPH_AGENT_ID) === agentId
  ...
  if (
    ...
    const repeatedTerminalReviewCount = countTrailingSameTaskClassifications(
      history,
      input.currentResult.selectedTaskId,
      agentId,
      [input.currentResult.completionClassification]
    );
