...
import { isDocumentationMode } from './taskFile';
import type {
...
const verificationGate: Gate<'verification'> = {
  id: 'verification',
...
const blockedOverrideGate: Gate<'blockedOverride'> = {
  id: 'blockedOverride',
...
export function composeMutationPlan(
  ...
  const suggestedValidationFromPlan =
    outputs.planValidation?.suggestedValidationFromPlan ?? state.suggestedValidationFromPlan;
  const validationToWrite =
    !selectedTask.validation && suggestedValidationFromPlan ? suggestedValidationFromPlan : null;
