...
import type { RalphPromptKind, RalphPromptTarget } from '../ralph/types';
...
export const REQUIRED_PROMPT_SECTIONS: PromptSectionName[] = [
  'strategyContext',
  'preflightContext',
  'objectiveContext',
  'taskContext',
...
export type PromptBudgetPolicyKey = `${RalphPromptKind}:${RalphPromptTarget}`;

export const CODEX_PROMPT_BUDGET_POLICIES: Record<PromptBudgetPolicyKey, PromptBudgetPolicy> = {
  'bootstrap:cliExec': {
...
export const CLAUDE_PROMPT_BUDGET_POLICIES: Record<PromptBudgetPolicyKey, PromptBudgetPolicy> = {
  'bootstrap:cliExec': {
...
export function buildPromptBudgetPolicy(
...
export function estimateTokenRange(estimatedTokens: number): {
  min: number;
  max: number;
} {
