...
import { compactList, formatOptional, roleContextProfile } from './promptText';

/**
 * Role-aware task/strategy context section builders for the Ralph prompt.
 *
 * Extracted from promptBuilder.ts so the prompt builder's core (template
 * rendering and budget policy) stays separate from the per-role context family.
 * Output is intentionally byte-identical to the previous inline implementation;
 * the golden prompt snapshots guard against drift.
 */
export interface RoleAwareTaskContextInput {
...
export function collectTaskLocalCodeContext(
  selectedTask: RalphTask | null,
...
export function buildTaskContext(input: RoleAwareTaskContextInput): string[] {
  const {
    kind,
    taskFile,
    taskCounts,
