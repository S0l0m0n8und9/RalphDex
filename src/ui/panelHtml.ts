...
function buildDeadLetterRecoveryCard(state: RalphDashboardState): string {
  const entries = state.dashboardSnapshot?.deadLetter.entries ?? [];
  if (entries.length === 0) {
    return '';
  }
...
type DashboardReadinessMode = 'noPrd' | 'emptyBacklog' | 'preflightBlocked' | 'ready';

function hasActionableWork(state: RalphDashboardState): boolean {
...
function getDashboardReadinessMode(state: RalphDashboardState): DashboardReadinessMode {
  if (!state.prdExists) {
    return 'noPrd';
  }
...
function buildHeroCard(state: RalphDashboardState): string {
  ...
  const summary = currentTask
    ? state.loopState === 'running'
      ? `${esc(state.workspaceName)} is executing ${esc(currentTask.id)} with the ${esc(state.agentRole)} role. Durable snapshot data remains live across overview, work, diagnostics, and settings.`
  ...
  return `<section class="hero-card">
    <div class="hero-topline">
      <div class="hero-headline">
        ...
        <div class="hero-summary">${esc(heroSummary)}</div>
      </div>
