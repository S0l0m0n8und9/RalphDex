import type { RalphDashboardState, RalphDashboardTask } from './uiTypes';
import {
  buildBaseCss,
  buildIterationRow,
  buildProgressBar,
...
function buildOverviewCounts(state: RalphDashboardState): string {
...
function buildTaskSearch(): string {
...
function buildFilterTabs(): string {
...
function buildSidebarTaskRow(task: RalphDashboardTask): string {
  ...
  let markers = '';
  if (task.isCurrent) markers += '<span class="sb-task-marker current" aria-label="Current task">▸</span>';
  if (task.blocker) markers += '<span class="sb-task-marker blocker" aria-label="Blocked">▒</span>';
  if (task.childIds.length > 0) markers += '<span class="sb-task-marker has-children" aria-label="Has subtasks">⊞</span>';
  if (task.dependsOn.length > 0 && !task.childIds.length) markers += '<span class="sb-task-marker" aria-label="Has dependencies">⊟</span>';

  return `<button type="button" class="sb-task-row ${statusClass}" data-task-id="${esc(task.id)}" data-task-status="${esc(task.status)}" data-task-priority="${esc(task.priority)}" data-task-blocker="${task.blocker ? esc(task.blocker) : ''}" aria-selected="${task.isCurrent ? 'true' : 'false'}" aria-expanded="false" aria-controls="${esc(detailId)}" tabindex="0">
    <span class="sb-task-glyph" aria-hidden="true">${glyph}</span>
    <span class="sb-task-id">${esc(task.id)}</span>
    <span class="sb-task-title">${esc(task.title)}</span>
    ${task.priority !== 'medium' ? `<span class="sb-task-priority">${esc(task.priority)}</span>` : ''}
    ${markers}
  </button>
  <div class="sb-task-detail" id="${esc(detailId)}" hidden>
    <dl>
...
function buildTaskList(state: RalphDashboardState): string {
...
function buildDeadLetterSection(state: RalphDashboardState): string {
  ...
  const rows = entries.slice(0, 5).map((e) => {
    const reason = e.diagnosticHistory.length > 0
      ? (e.diagnosticHistory[e.diagnosticHistory.length - 1] as { category?: string })?.category ?? 'unknown'
      : 'unknown';
    return `<div class="dl-entry">
      <div class="dl-entry-header">
        <span style="color:var(--bad);">✗</span>
        <span>${esc(e.taskId)}</span>
        <span style="flex:1; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${esc(e.taskTitle)}</span>
      </div>
      <div class="dl-entry-reason">${esc(reason)} · ${e.recoveryAttemptCount} attempts</div>
...
function buildRecentOutputs(state: RalphDashboardState): string {
  ...
  const rows = iters.map((iter) => buildIterationRow(iter)).join('\n');
  return `<div class="section-label">Recent Outputs</div>
    <div class="recent-outputs">${rows}</div>`;
}

function buildSeedForm(state: RalphDashboardState): string {
...
function buildSeedResult(state: RalphDashboardState): string {
  if (state.taskSeeding.phase === 'idle' || !state.taskSeeding.message) return '';
  ...
  if (state.taskSeeding.phase === 'success') {
    ...
    extra = `<div class="seed-success-actions">
      ...
      ${path ? `<span style="font-size:10px; color:var(--dim); margin-left:auto;" title="${esc(path)}">${esc(path)}</span>` : ''}
    </div>
  ...
  return `<div class="seed-result ${cls}">${esc(state.taskSeeding.message)}${extra}</div>`;
}
