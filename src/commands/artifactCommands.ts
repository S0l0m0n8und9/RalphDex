...
async function applyLatestDoctrineProposal(
  ...
  if (proposal.status !== 'proposed') {
    void vscode.window.showWarningMessage(
      `The latest doctrine proposal "${proposal.proposalId}" has already been ${proposal.status}. No further action taken.`
    );
    return false;
  }

  const protectedTargets = detectProtectedTargets(proposal);
  ...
  if (hasProtected) {
    const protectedConfirm = await vscode.window.showWarningMessage(
      `This doctrine proposal targets protected doctrine files (${protectedTargets.join(', ')}). These files govern invariants, boundaries, or agent rules and require extra care.\n\n${summary}`,
      { modal: true },
      'Apply Protected Doctrine Proposal'
    );
    ...
    const confirmed = await vscode.window.showWarningMessage(
      `Apply doctrine proposal to ${proposal.updates.length} update(s)?\n\n${summary}`,
      { modal: true },
      'Apply Proposal'
    );
    if (confirmed !== 'Apply Proposal') {
  ...
  const applicationResult = await applyDoctrineProposal({
    proposal,
    rootPath: workspaceFolder.uri.fsPath
  });
  ...
  const [reviewPaths] = await Promise.all([
    writeDoctrineProposalReviewArtifact({ artifactRootDir: inspection.paths.artifactDir, review }),
    writeUpdatedDoctrineProposalArtifact({ artifactRootDir: inspection.paths.artifactDir, proposal: updatedProposal })
  ]);
...
async function rejectLatestDoctrineProposal(
  workspaceFolder: vscode.WorkspaceFolder,
  stateManager: RalphStateManager,
  logger: Logger
): Promise<boolean> {
  ...
  const confirmed = await vscode.window.showWarningMessage(
    `Reject doctrine proposal "${proposal.proposalId}"? This will mark the proposal as rejected. No doctrine files will be modified.`,
    { modal: true },
    'Reject Proposal'
  );
    ...
    handler: async (progress) => {
      ...
      const choice = await vscode.window.showInformationMessage(
        vscode.workspace.isTrusted
          ...
          : 'Ralph status is available in the dashboard in limited mode. Raw report written to the output channel.',
        primaryAction,
        'Show Output'
      );
  ...
  registerCommand(context, logger, {
    ...
    handler: async (progress) => {
      ...
      if (queue.entries.length === 0) {
        void vscode.window.showInformationMessage(
          'No tasks are in the dead-letter queue. Use Ralphdex: Show Status to inspect task state.'
        );
        return;
      }
      ...
      const confirmed = await vscode.window.showWarningMessage(
        `Requeue task ${taskId} (${picked.description})? This removes it from the dead-letter queue and resets its status to todo.`,
        { modal: true },
        'Requeue'
      );
      ...
      const removed = await removeDeadLetterEntry(deadLetterPath, taskId);
      if (!removed) {
        void vscode.window.showWarningMessage(
          `Task ${taskId} was no longer in the dead-letter queue. Refresh status and try again.`
        );
        return;
      }
      ...
      const locked = await withTaskFileLock(inspection.paths.taskFilePath, undefined, async () => {
        ...
        task.status = 'todo';
        delete task.blocker;
        const next = bumpMutationCount(taskFile);
        await fs.writeFile(inspection.paths.taskFilePath, stringifyTaskFile(next), 'utf8');
      });
      ...
      if (locked.outcome === 'lock_timeout') {
        ...
        if (entry) {
          await appendDeadLetterEntry(deadLetterPath, entry);
        }
