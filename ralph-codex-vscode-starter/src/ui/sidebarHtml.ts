...
const CLASSIFICATION_CHAR: Record<string, string> = {
  complete: '█',
  partial_progress: '▓',
  no_progress: '░',
  blocked: '▒',
...
const LOOP_STATE_LABEL: Record<RalphUiLoopState, string> = {
  idle: '● idle',
  running: '▸ running',
  stopped: '■ stopped'
};

const PHASE_LABELS: RalphIterationPhase[] = [
...
function buildProgressBar(counts: RalphTaskCounts | null): string {
  if (!counts) {
...
function buildTaskRow(task: RalphDashboardTask, isRunning: boolean): string {
...
function buildPhaseTracker(currentPhase: RalphIterationPhase | null, currentIteration: number | null): string {
...
function buildIterationRow(iter: RalphDashboardIteration): string {
...
function buildDiagnostics(state: RalphDashboardState): string {
...
export function buildDashboardHtml(state: RalphDashboardState, nonce: string): string {
  ...
  // Sort tasks: current first, then in_progress, todo, blocked, done
...
<head>
  ...
  <style nonce="${nonce}">${buildCss()}</style>
</head>
<body>
  <div class="header">
  ...
  <div class="section-label">Tasks${state.taskCounts ? ` · ${state.taskCounts.done}/${state.taskCounts.todo + state.taskCounts.in_progress + state.taskCounts.blocked + state.taskCounts.done}` : ''}</div>
  <hr class="section-rule">
  ${buildProgressBar(state.taskCounts)}
  ${sortedTasks.length > 0
    ? sortedTasks.map((t) => buildTaskRow(t, isRunning)).join('\n')
  ...
  <script nonce="${nonce}">
    ...
    window.addEventListener('message', (event) => {
      ...
      if (msg.type === 'phase') {
        // Update phase tracker in place for smooth feedback
        const steps = document.querySelectorAll('.phase-step');
        const phases = ${JSON.stringify(PHASE_LABELS)};
        const activeIdx = phases.indexOf(msg.phase);
