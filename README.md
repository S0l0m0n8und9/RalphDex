...
The shipped dashboard and sidebar have one production ownership path: shared webview infrastructure lives under `src/webview/`, the bundled React shell lives under `src/webview-ui/`, and the VS Code surface adapters live under `src/ui/dashboardPanel.ts` and `src/ui/sidebarViewProvider.ts`. The React tree is the single renderer; if the local webview bundle has not been built, the view shows a small static "UI failed to load" page rather than a second implementation.
...
- [docs/artifact-registry-writer-inventory.md](docs/artifact-registry-writer-inventory.md): artifact writer registry adoption matrix
