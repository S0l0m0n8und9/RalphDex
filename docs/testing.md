...
- `scripts/run-e2e-pipeline-smoke.js`: opt-in temp-workspace pipeline smoke coverage that runs the shipped `codex exec` provider path against a deterministic fake executable, exercises the build, review, and SCM phases without network access, and asserts the resulting pipeline artifact contains a PR URL.
