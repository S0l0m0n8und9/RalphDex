...
The React dashboard exposes the same doctrine state without requiring command-palette discovery. The Doctrine card shows health (`missing`, `incomplete`, `invalid_evidence_index`, or `healthy`), protected files, bounded context budget usage, truncation, structural diagnostics, and pending proposal counts by risk when proposal artifacts exist. Its actions route only to existing safe commands or open-only helpers: initialize/repair the pack, reveal `.ralph/doctrine/`, open protected doctrine files, and open only the latest proposal artifact for review. It does not apply, reject, partially apply, edit Markdown inline, or invoke AI rewrite/regenerate doctrine flows.
