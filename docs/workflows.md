...
This tranche still does not add approval workflows, semantic memory mutation, prompt injection of doctrine content, autonomous doctrine rewriting, or auto-apply behavior. Providers may now emit bounded `doctrineUpdates` proposals in the completion report; Ralph validates them, classifies risk, and persists reviewable `doctrine-proposal.json` artifacts without mutating `.ralph/doctrine/*`. Preflight/status surfaces still treat missing or malformed doctrine as warning-level workspace diagnostics, not as task-selection blockers.
