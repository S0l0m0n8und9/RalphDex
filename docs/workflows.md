...
Doctrine now participates in prompt preparation as optional read-only context. Ralph collects a compact bounded excerpt from existing `.ralph/doctrine/*.md` files and injects a `Project Doctrine Context` section when entries are available; it never mutates doctrine files on this path. Missing or malformed doctrine remains warning-level in preflight/status diagnostics and does not block task selection or CLI launch by itself.

This tranche still does not add semantic memory mutation, autonomous doctrine rewriting, or doctrine auto-apply behavior. Providers may emit bounded `doctrineUpdates` proposals in the completion report; Ralph validates them, classifies risk, and persists reviewable `doctrine-proposal.json` artifacts without mutating `.ralph/doctrine/*`.

The React dashboard exposes the same doctrine state without requiring command-palette discovery. The Doctrine card shows health (`missing`, `incomplete`, `invalid_evidence_index`, or `healthy`), protected files, bounded context budget usage, truncation, structural diagnostics, and pending proposal counts by risk when proposal artifacts exist. The proposal review surface lists pending proposal artifacts, shows bounded detail for each update, and lets the operator apply all updates, reject the proposal, open the target file, or partially apply selected update indexes. Partial apply is intentionally bounded to whole proposed updates; it is not a Markdown editor or free-form rewrite surface. Protected targets (`invariants.md`, `boundaries.md`, and `agents.md`) require an explicit protected-approval action before dashboard apply mutates doctrine files. Review outcomes persist as the existing doctrine proposal review artifacts (`*.review.json` and `*.review.md`) and update the canonical proposal status.
...
When doctrine context is present, `prompt-evidence.json` records doctrine-context observability metadata (`includedFiles`, `omittedFiles`, counts, and budget truncation flags). `Show Status` then surfaces doctrine-context availability, included/omitted counts, and the latest doctrine-context evidence path.
...
- The dashboard `Work` tab and the sidebar both include a task-seeding intake form. Both paths send the entered epic text through the same provider-backed task-seeding helper used by `Ralphdex: Add Task` and `Ralphdex: Seed Tasks from Feature Request`, then expose the created-task count plus follow-up actions such as `Ralphdex: Show Tasks` and dashboard refresh.
- `Ralphdex: Show Tasks` focuses the sidebar task tree, which groups active durable tasks separately from dead-letter entries and surfaces per-task plan, diagnostic, stale-claim, and requeue affordances through the existing command layer.
...
- `off`: planning remains context-only; execution always proceeds. Existing `task-plan.json` artifacts remain advisory context even when older plans do not contain `planningInput`; Ralph does not force regeneration solely for missing freshness metadata in this mode.
...
> **Maturity: beta** — Azure-backed providers are functional, but operators must supply resource metadata and auth credentials in the process environment before launch.

RalphDex supports three Azure-related execution paths:

- `copilot-byok` — runs GitHub Copilot CLI; auth is fully external (BYOK). RalphDex shapes the `COPILOT_PROVIDER_*` environment and passes them through. No credentials are resolved by RalphDex.
- `copilot-foundry` — alias for `copilot-byok` that forces `providerType: azure`. Convenience preset for operators who always target Azure OpenAI.
- `azure-foundry` — uses RalphDex's direct HTTPS Azure provider with RalphDex-owned auth resolution.
...
For `copilot-byok` / `copilot-foundry`:

| Setting | Purpose |
|---|---|
| `ralphCodex.copilotFoundry.azure.resourceName` + `azure.deployment` | Azure resource and deployment — RalphDex derives the base URL |
| `ralphCodex.copilotFoundry.baseUrlOverride` | Full base URL override; takes precedence over derived URL; required when `providerType` is not `azure` |
| `ralphCodex.copilotFoundry.model` | Model name injected as `COPILOT_MODEL` in the child process |
| `ralphCodex.copilotFoundry.requiredApiKeyEnvVar` | Name of the env var the operator must set. Checked at preflight — value is never read by RalphDex. |

### Authentication

**`azure-foundry`** supports three RalphDex-owned auth modes configured via `ralphCodex.azureFoundry.auth.mode`:

1. `az-bearer` — Ralph acquires a bearer token via `@azure/identity` at runtime using `DefaultAzureCredential`, supporting local Azure-authenticated environments and Managed Identity flows.
...
Use `Ralphdex: Set Provider Secret` and `Ralphdex: Clear Provider Secret` to manage `vscode-secret` values.

**`copilot-byok` / `copilot-foundry`** have no RalphDex-owned auth. The operator process must supply credentials before launch. RalphDex sets these Copilot CLI BYOK environment variables:

| Variable | Source |
|---|---|
| `COPILOT_PROVIDER_TYPE` | `ralphCodex.copilotFoundry.providerType` (forced to `azure` for `copilot-foundry`) |
| `COPILOT_PROVIDER_BASE_URL` | Derived from `azure.resourceName` + `azure.deployment`, or `baseUrlOverride` |
| `COPILOT_MODEL` | `ralphCodex.copilotFoundry.model` (omitted if blank) |
| `COPILOT_OFFLINE` | `true` when `ralphCodex.copilotFoundry.offline` is enabled |

The API key or bearer token is not injected — it must already be present in the process environment via the operator's deployment setup.
