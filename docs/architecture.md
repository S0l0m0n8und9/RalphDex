...
- [Event Journal Coverage](event-journal-coverage.md)
...
- `src/ralph/artifactRegistry.ts`: canonical artifact registry and relationship index — a single `.ralph/artifacts/index.json` recording typed metadata (`type`, `path`, `createdAt`, `runId`, `taskId`, `agentId`, `agentRole`, `provider`, `retentionClass`, `pinned`, `related`) for every registered artifact, with lock-guarded `registerArtifacts`/`reconcileArtifactRegistry` writes and a pure `queryArtifacts` lookup by run/task/type/provider/role. Additive: latest-pointer files stay authoritative for compatibility; the registry is the richer query source for dashboard/sidebar navigation and cleanup decisions. `registerIterationArtifactSet` (in `artifactStore.ts`) registers each iteration's persisted artifacts; cleanup reconciles the index so it stays consistent with disk.
- `src/ralph/cleanupManifest.ts`: cleanup manifest model (issue #72) — typed `RalphCleanupManifest` (`preview`/`applied`) recording what was/would be deleted, retained, protected, and repaired, with a markdown renderer. `stateManager.previewRuntimeArtifactCleanup` returns the dry-run manifest without touching disk; `cleanupRuntimeArtifacts` repairs latest-pointer surfaces, reconciles the registry, and persists the applied manifest as `cleanup-manifest.json`/`.md`. The deletion-selection logic is shared for both generated artifacts (`selectDeletableEntries`/`buildGeneratedArtifactCleanupPlan`) and provenance bundles (`selectDeletableBundleIds`) in `artifactRetention.ts`, so preview and apply can never drift.
- `src/ralph/prdReconciliation.ts`: PRD ↔ backlog reconciliation (issue #71) — deterministic, **review-only** drift detection between `.ralph/prd.md` and `.ralph/tasks.json` (`stale_prd_task_reference`, `orphan_active_task`, `duplicate_active_task`), with a markdown renderer. Never mutates `tasks.json`. `Show Status` runs it, writes a `prd-reconciliation.json`/`.md` proposal under `.ralph/artifacts`, and appends a compact findings section to the status report.
...
- `src/ralph/eventJournal.ts`: typed, append-only runtime event journal — discriminated-union events (task/provider/verifier/review/SCM/recovery/workflow transitions), a per-run JSONL writer with monotonic sequence numbers under `.ralph/artifacts/runs/<runId>/events.jsonl`, and pure reducers that fold a journal into a current run/task snapshot. Additive substrate beside `tasks.json`/`state.json` for provenance, diagnostics, replay, and dashboard snapshots; consumers migrate onto it incrementally. Coverage expectations live in [docs/event-journal-coverage.md](event-journal-coverage.md).
- `src/ralph/runTimeline.ts`: operator trust timeline (issue #73) — pure projections of durable state into a **pre-run execution intent preview** (`buildExecutionIntentPreview`: provider, autonomy mode, verifier stack, SCM mode, possible task mutations) and a **post-run trust timeline** (`buildRunTrustTimeline`: a chronological mutation timeline + auto-remediation audit + stop reason folded from the #68 event journal). The dashboard data loader builds the intent from the effective config + selected task, builds the timeline from the latest run's `events.jsonl`, and attaches file-level repository changes from the run's durable `diff-summary.json` when present. `buildDashboardSnapshot` projects these into a `runTimeline` section rendered by the React `RunTimelinePanel`. No string-rendered UI path.
...
- `test/ui/` and `test/webview/` own regression coverage for the React shell bootstrap, component behaviour, and shared host wiring. `test/ui/` runs under a headless DOM harness (`test/register-dom.cjs`: jsdom + a mocked `acquireVsCodeApi`) loaded only for that suite, so components can be rendered with `@testing-library/react` and driven with real events — covering click handlers, effects, and the full webview↔host message round-trip (`test/ui/appRoundTrip.test.tsx`) without launching a browser or Extension Development Host. Heavier in-browser EDH automation stays out of scope per `docs/boundaries.md`.
