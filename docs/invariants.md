...
| PRD generation / backlog seeding / bootstrap append | `generateProjectDraft`, `seedTasksFromRequest`, or command-local drafts → `appendNormalizedTasksToFile` → `normalizeNewTask` | Initialize Workspace and New Project append through the shared persistence helper, and `Ralphdex: Add Task` now seeds flat backlog tasks from a high-level request through the same normalization boundary. Rich producer fields survive AI generation, seeding, and fallback/bootstrap paths. |
