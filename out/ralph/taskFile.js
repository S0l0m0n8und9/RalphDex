...
exports.selectNextTaskForRole = selectNextTaskForRole;
...
exports.applySuggestedChildTasksWithinLock = applySuggestedChildTasksWithinLock;
...
const taskFileLocator_1 = require("./taskFileLocator");
...
function inspectTaskFileText(raw) {
    ...
    const entryLocations = (0, taskFileLocator_1.extractTaskEntryLocations)(raw);
    ...
    const correctedTasks = migratedTasks.map((task) => {
        ...
        if (task.parentId && task.status === 'todo' && statusById.get(task.parentId) === 'done') {
            diagnostics.push(createTaskGraphDiagnostic('auto_corrected_parent_reference', `${taskLabel(task)} had parentId "${task.parentId}" which is already done; parentId stripped to prevent a stuck state.`, {
                severity: 'warning',
                taskId: task.id,
                relatedTaskIds: [task.parentId],
                location: task.source
...
async function taskArtifactExists(artifactsDir, taskId, fileName) {
    try {
...
async function selectNextTaskForRole(taskFile, agentRole, artifactsDir, options) {
    ...
    if (agentRole === 'reviewer') {
        const sortByPriority = (tasks) => [...tasks].sort((l, r) => taskPriorityOrder(l) - taskPriorityOrder(r));
        const doneTasks = sortByPriority(taskFile.tasks.filter((task) => task.status === 'done'));
...
async function applySuggestedChildTasksWithinLock(taskFilePath, parentTaskId, suggestedChildTasks) {
