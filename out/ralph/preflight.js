 ...
 */
function hasUntrackedProjectBaseline(status) {
    if (!status?.available) {
        return false;
    }
...
function collectCodexModelReadinessDiagnostics(config) {
    const diagnostics = [];
    ...
    for (const candidate of candidates) {
        ...
        diagnostics.push(createDiagnostic('codexAdapter', 'warning', 'codex_model_chatgpt_auth_may_be_unsupported', `Codex model ${model} from ${candidate.source} may be unsupported when the Codex CLI is authenticated with a ChatGPT account. Confirm the account supports this model or choose a supported Codex model before starting a live iteration.`));
    }
    return diagnostics;
}
...
function collectCopilotByokReadinessDiagnostics(config) {
    const diagnostics = [];
    ...
    if (envVarName) {
        ...
        if (isPresent) {
            diagnostics.push(createDiagnostic('codexAdapter', 'info', 'copilot_byok_api_key_present', `copilot-byok: Required API key env var ${envVarName} is present in the current process environment.`));
        }
        else {
            diagnostics.push(createDiagnostic('codexAdapter', 'warning', 'copilot_byok_api_key_absent', `copilot-byok: Required API key env var ${envVarName} is not set. The child process must inherit it from the operator environment before launch.`));
        }
    }
    return diagnostics;
}
