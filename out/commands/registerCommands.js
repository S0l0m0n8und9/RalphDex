...
function registerCommands(context, logger, broadcaster, panelManager) {
    ...
    async function ensureRealPrdOrOpenWizard(workspaceFolder, config, progress) {
        ...
        void vscode.window.showWarningMessage('RalphDex needs a real PRD before running. The PRD wizard is open; finish it, then start the command again.');
        return false;
    }
        ...
        handler: async (progress) => {
            ...
            if (missingWorkspaceFiles.length > 0) {
                const relativeMissing = missingWorkspaceFiles
                    ...
                    .join(', ');
                void vscode.window.showWarningMessage(`Doctrine pack initialization requires an established Ralph workspace. Missing: ${relativeMissing}. Run "Ralphdex: Bootstrap Ralph Workspace" first.`);
                return;
            }
