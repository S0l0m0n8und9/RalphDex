...
const prdReadinessGate_1 = require("./prdReadinessGate");
...
function summarizeRelativePaths(rootPath, targetPaths) {
...
async function openWorkspaceFile(targetPath) {
    const document = await vscode.workspace.openTextDocument(vscode.Uri.file(targetPath));
    await vscode.window.showTextDocument(document, { preview: false });
}
async function openDoctrineFile(workspaceFolder, fileName) {
    const targetPath = path.join(workspaceFolder.uri.fsPath, doctrine_1.DOCTRINE_ROOT_RELATIVE, fileName);
    if (!(await (0, fs_1.pathExists)(targetPath))) {
        void vscode.window.showWarningMessage(`Doctrine file ${doctrine_1.DOCTRINE_ROOT_RELATIVE}/${fileName} does not exist yet. Run "Ralphdex: Initialize Doctrine Pack" first.`);
        return;
    }
    await openWorkspaceFile(targetPath);
}
function buildDoctrinePackMessage(rootPath, result) {
...
async function initializeFreshWorkspace(rootPath) {
    ...
    await fs.writeFile(prdPath, prdReadinessGate_1.RALPH_PRD_PLACEHOLDER, 'utf8');
...
async function findMissingRalphWorkspaceFiles(paths) {
...
function registerCommands(context, logger, broadcaster, panelManager) {
    ...
    async function ensurePrdReadyOrOpenWizard(workspaceFolder, config, progress) {
        const gate = await (0, prdReadinessGate_1.evaluatePrdReadinessGate)({
            workspaceFolder,
            config,
            stateManager,
            logger
        ...
        const suppressedHash = context.workspaceState.get(suppressionKey);
        const isSuppressed = suppressedHash === gate.readiness.prdHash;
        if (!isSuppressed) {
            void vscode.window.showWarningMessage(`PRD has ${blockerCount} structural finding(s) flagged by readiness analysis. Continuing the command — open the wizard or the readiness report if you want to address them.`, 'Open Readiness Report', 'Open PRD Wizard', "Don't show again").then((choice) => {
    ...
    async function runPipelineFromPhase(startPhase, artifact, workspaceFolder, config, paths, progress) {
        ...
        const { artifact: finalArtifact, status: loopStatus } = await (0, pipelineDriver_1.drivePipelineRun)({
            ...
            runners: {
                ...
                runReview: async () => (await vscode.commands.executeCommand('ralphCodex.runReviewAgent')),
                runScm: async () => (await vscode.commands.executeCommand('ralphCodex.runScmAgent'))
            },
        ...
        void vscode.window.showInformationMessage(`Ralph pipeline ${finalArtifact.runId} finished with status: ${loopStatus}. Root task: ${finalArtifact.rootTaskId} (${finalArtifact.decomposedTaskIds.length} subtask(s)).${prSuffix}`);
    }
        ...
        handler: async (progress) => {
            ...
            if (missingWorkspaceFiles.length > 0) {
                const relativeMissing = missingWorkspaceFiles
                    ...
                    .join(', ');
                void vscode.window.showWarningMessage(`Doctrine pack initialization requires an established Ralph workspace. Missing: ${relativeMissing}. Run "Ralphdex: Initialize Workspace" first.`);
                return;
            }
            const result = await (0, doctrine_1.createDoctrinePack)(workspaceFolder.uri.fsPath);
            ...
            void vscode.window.showInformationMessage(buildDoctrinePackMessage(workspaceFolder.uri.fsPath, result));
        }
    });
    registerCommand(context, logger, {
        ...
        handler: async (progress) => {
            const workspaceFolder = await withWorkspaceFolder();
            const doctrineDir = path.join(workspaceFolder.uri.fsPath, doctrine_1.DOCTRINE_ROOT_RELATIVE);
            progress.report({ message: 'Opening the Ralph doctrine folder' });
            if (!(await (0, fs_1.pathExists)(doctrineDir))) {
