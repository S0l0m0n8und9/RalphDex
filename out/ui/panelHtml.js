...
function renderLatestArtifactActions(buttonClass = 'btn') {
      ...
      <div class="settings-grid">
        ...
        ${section.id === 'provider' ? `
          <div class="inline-actions" style="grid-column: 1 / -1; margin-top: -2px; margin-bottom: 4px;">
            <button class="btn" data-command="ralphCodex.testCurrentProviderConnection" aria-label="Test connection for ${(0, htmlHelpers_1.esc)(currentProvider)} provider"><span class="btn-label">${(0, htmlHelpers_1.esc)(getProviderTestLabel(currentProvider))}</span><span class="btn-spinner"></span></button>
...
function buildDiagnosisSection(state) {
    ...
    <div class="failure-meta">
      <div><strong>${(0, htmlHelpers_1.esc)(diagnosis.taskId)}</strong> · ${(0, htmlHelpers_1.esc)(diagnosis.taskTitle)}</div>
      <div><strong>Category</strong> ${(0, htmlHelpers_1.esc)(diagnosis.category)} · <strong>Confidence</strong> ${(0, htmlHelpers_1.esc)(diagnosis.confidence)}</div>
      <div><strong>Cause</strong> ${(0, htmlHelpers_1.esc)(diagnosis.summary)}</div>
      <div><strong>Suggested action</strong> ${(0, htmlHelpers_1.esc)(diagnosis.suggestedAction)}</div>
      <div><strong>Recovery attempts</strong> ${diagnosis.recoveryAttemptCount ?? 0}</div>
      ${diagnosis.remediationSummary ? `<div><strong>Remediation</strong> ${(0, htmlHelpers_1.esc)(diagnosis.remediationSummary)}</div>` : ''}
      ${diagnosis.retryPromptAddendum ? `<div><strong>Retry addendum</strong> ${(0, htmlHelpers_1.esc)(diagnosis.retryPromptAddendum)}</div>` : ''}
      <div><strong>failure-analysis.json</strong> ${(0, htmlHelpers_1.esc)(diagnosis.failureAnalysisPath ?? 'none')}</div>
      <div><strong>recovery-state.json</strong> ${(0, htmlHelpers_1.esc)(diagnosis.recoveryStatePath ?? 'none')}</div>
      <div class="inline-actions">
...
function buildHeroHealthCell(label, value, subtitle, fillPercent, fillColor = 'var(--accent)') {
...
function buildFirstRunReadinessSection(state) {
...
function buildSettingsTab(state) {
    ...
    <div class="card">
      ...
      <div class="btn-grid">
        ...
        <button class="btn" data-command="ralphCodex.openSettings"><span class="btn-label">Open Settings UI</span><span class="btn-spinner"></span></button>
      </div>
    </div>
