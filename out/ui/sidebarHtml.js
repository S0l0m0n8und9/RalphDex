...
function buildCurrentTaskCard(state) {
    ...
    ${id ? `<div class="current-task-id">${(0, htmlHelpers_1.esc)(id)}</div>` : ''}
    ${title ? `<div class="current-task-title">${(0, htmlHelpers_1.esc)(title)}</div>` : ''}
    ${status ? `<div class="current-task-meta">${(0, htmlHelpers_1.esc)(status.replace(/_/g, ' '))}${blocker ? ` · ${(0, htmlHelpers_1.esc)(blocker)}` : ''}</div>` : ''}
    ${countLine ? `<div class="current-task-meta">${(0, htmlHelpers_1.esc)(countLine)}</div>` : ''}
