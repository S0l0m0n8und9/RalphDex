...
function buildCurrentTaskCard(state) {
    ...
    ${recoveryEntry ? `<div class="current-task-meta">Recovery: ${(0, htmlHelpers_1.esc)(recoveryEntry.taskTitle)}</div>` : ''}
...
function buildRunControl(state) {
...
function buildSidebarStatusText(state, stateLabel) {
    if (state.loopState === 'running') {
        return `<div class="sidebar-status-text">Ralph is working — ${(0, htmlHelpers_1.esc)(stateLabel)}</div>`;
    }
    if (!state.prdExists) {
        return '<div class="sidebar-status-text warn">PRD required before running</div>';
    }
    if (!state.preflightReady) {
        return `<div class="sidebar-status-text warn">Readiness blocked — ${(0, htmlHelpers_1.esc)(state.preflightSummary)}</div>`;
    }
...
<body>
  ...
  ${phaseIndicator}
  ${state.taskCounts ? (0, htmlHelpers_1.buildProgressBar)(state.taskCounts) : ''}
